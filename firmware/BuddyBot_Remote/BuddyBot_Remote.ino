 ...
 * ════════════════════════════════════════════════════════════════════
 *  WIRING
 *  OLED XC3726:  CS→10  DC→9  RES→5  SDA→MOSI pad  SCL→SCK pad
 *                VCC→3V  GND→GND  (underside pads of XC3812)
 *  Buttons (INPUT_PULLUP to GND):
 *    UP→D11  DOWN→D12  LEFT→D13  RIGHT→A2
 *  RF 433MHz: DATA→D6  VCC→3.3V  GND→GND
 *  Battery:   LiPo+ → 2x100kΩ divider → A0
 *
 *  CONTROLS
...
bool eStop=false,driveFwd=false,driveRev=false,menuConsumed=false;
...
struct Btn { uint8_t pin; bool state,rawPrev,pressed,released,held; unsigned long downAt; };
Btn btns[NUM_BTNS];
#define B_UP 0
#define B_DOWN 1
#define B_LEFT 2
...
void initBtns() {
  ...
  for(int i=0;i<NUM_BTNS;i++){btns[i]={pins[i],HIGH,HIGH,false,false,false,0};pinMode(pins[i],INPUT_PULLUP);}
}
...
bool longHeld(int i,unsigned long ms=LONG_MS){return btns[i].held&&(millis()-btns[i].downAt>=ms);}
...
void drawGrid(){for(int x=0;x<OLED_W;x+=16)oled.drawFastVLine(x,0,OLED_H,0x0421u);for(int y=0;y<OLED_H;y+=16)oled.drawFastHLine(0,y,OLED_W,0x0421u);}
...
void drawC(int y,const char* s,uint16_t c,uint8_t sz=1){
  ...
  oled.setCursor((OLED_W-(int)strlen(s)*6*sz)/2,y);oled.print(s);
}

void glowBar(int x,int y,int w,int h,float frac,uint16_t col){
  oled.fillRoundRect(x,y,w,h,2,C_SURF);oled.drawRoundRect(x,y,w,h,2,col);
  int fill=(int)(frac*(w-2));
  if(fill>0){oled.fillRect(x+1,y+1,fill,h-2,col);oled.drawFastHLine(x+1,y+1,fill,C_WHITE);}
}

void drawBat(int x,int y){
  uint16_t c=batPct>50?C_MINT:batPct>20?C_AMBER:C_CORAL;
  oled.drawRoundRect(x,y,20,10,2,c);oled.fillRect(x+20,y+3,2,4,c);
  int f=(int)(batPct/100.0f*17.0f);if(f>0)oled.fillRect(x+2,y+2,f,6,c);
  oled.setTextSize(1);oled.setTextColor(c);
  char b[5];snprintf(b,5,"%d%%",batPct);oled.setCursor(x+24,y+1);oled.print(b);
}

void drawHdr(const char* title,uint16_t col){
  oled.fillRect(0,0,OLED_W,16,C_BK);
  oled.drawFastHLine(0,15,OLED_W,col);oled.drawFastHLine(0,14,OLED_W,C_SURF);
  oled.drawFastHLine(0,13,OLED_W,C_XCYAN);
  oled.setTextSize(1);oled.setTextColor(col);
  oled.setCursor((OLED_W-(int)strlen(title)*6)/2,4);oled.print(title);
  drawBat(OLED_W-52,3);
}

void drawMenu(int sel,int count,const char** names,const uint16_t* cols){
  for(int i=0;i<count;i++){
    bool act=(i==sel);int y=20+i*24;
    uint16_t bg=act?cols[i]:C_BK,fg=act?C_BK:cols[i];
    oled.fillRect(4,y,120,20,bg);oled.drawRect(4,y,120,20,cols[i]);
    oled.drawRect(5,y+1,118,18,cols[i]);
    if(act){oled.drawFastHLine(8,y,112,C_WHITE);oled.drawFastHLine(8,y+19,112,C_WHITE);}
    oled.setTextSize(1);oled.setTextColor(fg);
    oled.setCursor((OLED_W-(int)strlen(names[i])*6)/2,y+6);oled.print(names[i]);
  }
  drawC(118,"UP/DN=scroll  LEFT=ok",C_DGRAY);
}

void driveBlock(int cx,int cy,int w,int h,const char* lbl,bool active,uint16_t col){
  uint16_t bg=active?col:C_BK,fg=active?C_BK:col;
  oled.fillRect(cx-w/2,cy-h/2,w,h,bg);oled.drawRect(cx-w/2,cy-h/2,w,h,col);
  oled.drawRect(cx-w/2+1,cy-h/2+1,w-2,h-2,col);
  if(active){oled.drawFastHLine(cx-w/2+2,cy-h/2,w-4,C_WHITE);oled.drawFastHLine(cx-w/2+2,cy+h/2-1,w-4,C_WHITE);}
  oled.setTextSize(1);oled.setTextColor(fg);oled.setCursor(cx-(int)strlen(lbl)*3,cy-4);oled.print(lbl);
}

void drawDriveScreen(){
  bool up=driveFwd,dn=driveRev,l=btns[B_LEFT].held,r=btns[B_RIGHT].held;
  ...
  oled.fillCircle(CX,CY,9,cc);oled.drawCircle(CX,CY,9,C_WHITE);oled.drawCircle(CX,CY,10,cc);
  if(eStop){oled.setTextSize(1);oled.setTextColor(C_WHITE);oled.setCursor(CX-6,CY-4);oled.print("E!");}
  if(eStop){oled.fillRect(0,17,OLED_W,11,C_CORAL);drawC(19,"!! E-STOP ACTIVE !!",C_WHITE);}
  char spb[8];snprintf(spb,sizeof(spb),"%s",spdNames[spdIdx]);
  oled.fillRoundRect(2,108,56,14,3,C_BK);oled.drawRoundRect(2,108,56,14,3,spdCols[spdIdx]);
  oled.setTextSize(1);oled.setTextColor(spdCols[spdIdx]);oled.setCursor(5,111);oled.print("SPD:");oled.print(spb);
  oled.fillRoundRect(62,108,64,14,3,C_BK);oled.drawRoundRect(62,108,64,14,3,modeCols[modeIdx]);
  oled.setTextSize(1);oled.setTextColor(modeCols[modeIdx]);oled.setCursor(65,111);oled.print(modeNames[modeIdx]);
  drawC(124,"HOLD-DN=menu",C_DGRAY);
}

void bootSplash(){
  oled.fillScreen(C_BK);randomSeed(42);
  for(int i=0;i<40;i++){uint16_t sc=random(3)==0?C_DGRAY:random(2)?C_MGRAY:C_LGRAY;oled.drawPixel(random(OLED_W),random(OLED_H),sc);}
  for(int i=0;i<=OLED_W;i+=4){oled.drawPixel(i,0,C_CYAN);oled.drawPixel(OLED_W-1-i,OLED_H-1,C_CYAN);delay(3);}
  for(int i=0;i<=OLED_H;i+=4){oled.drawPixel(0,i,C_PURPLE);oled.drawPixel(OLED_W-1,OLED_H-1-i,C_PURPLE);delay(3);}
  hexCorners(4,4,OLED_W-8,OLED_H-8,C_CYAN,10);hexCorners(6,6,OLED_W-12,OLED_H-12,C_DCYAN,6);
  drawC(22,"BUDDYBOT",C_CYAN,2);drawC(40,"REMOTE",C_CYAN,2);
  oled.drawFastHLine(20,58,OLED_W-40,C_PURPLE);
  drawC(62,"v3.0",C_PURPLE,1);drawC(72,"XC3812  SAMD21",C_DGRAY,1);
  oled.drawFastHLine(20,82,OLED_W-40,C_DCYAN);
  drawC(88,"Reinsma Innovations",C_DCYAN,1);drawC(98,"GENERATIVE AI",C_MGRAY,1);
  drawC(110,"STARTING...",C_MGRAY,1);
  for(int p=0;p<=100;p+=5){glowBar(16,120,OLED_W-32,6,p/100.0f,C_CYAN);delay(28);}
  delay(300);
  for(int b=0;b<3;b++){oled.fillScreen(C_CYAN);delay(35);oled.fillScreen(C_BK);delay(55);}
}

void processDrive(){
  static unsigned long lastRFSend=0;unsigned long now=millis();
  if(btns[B_LEFT].held&&btns[B_RIGHT].held&&longHeld(B_LEFT,LONG_MS)&&longHeld(B_RIGHT,LONG_MS)){
    eStop=!eStop;rf.send(eStop?RF_ESTOP:RF_STOP,24);
    if(eStop)driveFwd=driveRev=false;
    btns[B_LEFT].held=btns[B_RIGHT].held=false;redraw=true;delay(300);return;
  }
  if(eStop)return;
  if(longHeld(B_DOWN,MENU_MS)&&!menuConsumed){
    menuConsumed=true;driveFwd=driveRev=false;rf.send(RF_STOP,24);
    curScr=SCR_MODE;redraw=true;
    oled.fillRect(0,16,OLED_W,OLED_H-16,C_BK);drawGrid();return;
  }
  if(!btns[B_DOWN].held)menuConsumed=false;
  if(btns[B_UP].pressed){driveRev=false;driveFwd=!driveFwd;redraw=true;}
  if(btns[B_DOWN].pressed&&!menuConsumed){driveFwd=false;driveRev=!driveRev;redraw=true;}
  if(btns[B_LEFT].pressed||btns[B_RIGHT].pressed){driveFwd=driveRev=false;redraw=true;}
  ...
  if(now-lastRFSend>=RF_MS){rf.send(cmd,24);lastRFSend=now;if(cmd!=lastRFCode){lastRFCode=cmd;redraw=true;}}
}
...
bool menuNav(uint8_t& idx,uint8_t count){
  ...
  if(btns[B_RIGHT].pressed){
    ...
    oled.fillRect(0,16,OLED_W,OLED_H-16,C_BK);drawGrid();drawHdr("BUDDYBOT RC",C_CYAN);
  }
  return false;
}
...
void setup(){
  initBtns();
  rf.enableTransmit(RF_PIN);rf.setProtocol(1);rf.setPulseLength(189);rf.setRepeatTransmit(5);
  analogReadResolution(10);analogReference(AR_DEFAULT);
  oled.begin();
  oled.setRotation(2);   // flip 180 degrees — corrects upside-down display
  bootSplash();
  oled.fillScreen(C_BK);drawGrid();drawHdr("BUDDYBOT RC",C_CYAN);
  ...
  switch(curScr){
    case SCR_DRIVE:{
      ...
      if(redraw||f!=pF||r!=pR||l!=pL||ri!=pRi||eStop!=pE){
        oled.fillRect(0,16,OLED_W,OLED_H-16,C_BK);drawGrid();drawDriveScreen();
    ...
    case SCR_MODE:
      ...
      if(menuNav(modeIdx,4)){
        rf.send(modeCodes[modeIdx],24);curScr=SCR_SPEED;spdIdx=1;redraw=true;
        oled.fillRect(0,16,OLED_W,OLED_H-16,C_BK);drawGrid();drawHdr("SELECT SPEED",C_AMBER);
      }
    ...
    case SCR_SPEED:
      ...
      if(menuNav(spdIdx,3)){
        rf.send(spdCodes[spdIdx],24);curScr=SCR_DRIVE;redraw=true;
        oled.fillRect(0,16,OLED_W,OLED_H-16,C_BK);drawGrid();drawHdr("BUDDYBOT RC",C_CYAN);
      }
      if(redraw){
        ...
        for(int i=0;i<3;i++){
          ...
          oled.fillRect(8,y,112,26,sel?spdCols[i]:C_BK);oled.drawRect(8,y,112,26,spdCols[i]);
          oled.drawRect(9,y+1,110,24,spdCols[i]);
          if(sel){oled.drawFastHLine(12,y,104,C_WHITE);oled.drawFastHLine(12,y+25,104,C_WHITE);}
          oled.setTextSize(2);oled.setTextColor(sel?C_BK:spdCols[i]);
          oled.setCursor((OLED_W-(int)strlen(spdNames[i])*12)/2,y+9);oled.print(spdNames[i]);
        }
        drawC(118,"UP/DN=scroll  LEFT=ok",C_DGRAY);redraw=false;
      }
      break;
  }
  delay(8);
